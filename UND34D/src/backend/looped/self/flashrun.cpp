...
namespace big
{
	...
	static bool flash_run_state = false;
	void looped::self_flash_run()
	{
		if (g->self.flash_run && PAD::IS_CONTROL_PRESSED(0, (int)ControllerInputs::INPUT_SPRINT))
		{
			ptfx::flashrunPTFX();
		...
		else if (!g->self.flash_run && g->self.flash_run != flash_run_state)
		{
		...
		flash_run_state = g->self.flash_run;
	}
}